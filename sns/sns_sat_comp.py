...
def main():
    ...
    topo = Network.from_topology_builder_svc(
        f"http://localhost:8000/topology_builder/min_dist_topo_builder/iridium?t={now.strftime('%Y-%m-%d %H:%M:%S %z').replace('+', '%2B')}&no_gs_s=3"
    )
    ...
    for source, s_info in topo.get_leo_satellites():
        for i, target in enumerate(list(topo.graph.adj[source])):
            ...
            if topo.graph.nodes[target]["type"] == NodeTypes.LEO_SATELLITE:
                wire.out = topo.graph.nodes[target]["sat"]
            else:
    ...
    for gs, info in topo.get_GSs():
        ...
        for _, pg in info["packet_generator"].items():
            pg.out = topo.graph.nodes[upstream_sat]["sat"]
        ...
        for target, _ in topo.get_GSs():
            ...
            for hop in hops_but_first:
                if topo.graph.nodes[prev_hop]["type"] == NodeTypes.GROUD_STATION:
                    ...
                    print(
                        f"link : {prev_hop} -{topo.graph[prev_hop][hop]['out_port']}-> {hop}"
                    )
...
if __name__ == "__main__":
    ...
    pg.out = sat
    sat.switch.demux.fib = {1: 0}
    sat.switch.ports[0].out = ps
